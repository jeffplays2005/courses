    ...
    def percolate_up(self, index):
        while index // 2 > 0:
            if self.binary_heap[index] > self.binary_heap[index // 2]:
                tmp = self.binary_heap[index // 2]
                self.binary_heap[index // 2] = self.binary_heap[index]
                self.binary_heap[index] = tmp
            index = index // 2
    
    def percolate_down(self, index):
        while (index * 2) <= self.size:
            largest_child = self.get_larger_child_index(index)
            if self.binary_heap[index] < self.binary_heap[largest_child]:
                tmp = self.binary_heap[index]
                self.binary_heap[index] = self.binary_heap[largest_child]
                self.binary_heap[largest_child] = tmp
            index = largest_child
